...
function ssf_site_home_shortcode(): string
{
    ...
    <section class="ssf-section">
        <div class="ssf-wrap">
            ...
            <div class="ssf-home-card-grid">
                <?php echo ssf_site_home_action_card('Ansöka som fartygsombud', 'Testa om ditt fartyg kan gå vidare till ansökan', home_url('/ansokan/')); ?>
                <?php echo ssf_site_home_action_card('Bli stödmedlem', 'Stöd arbetet för Sveriges segelfartyg', home_url('/medlemskap/')); ?>
                <?php echo ssf_site_home_action_card('Kontakta SSF', 'Ställ en fråga till förbundet', home_url('/kontakta-oss/')); ?>
            </div>
        </div>
    </section>
    ...
    <section class="ssf-section ssf-section--blue">
        <div class="ssf-wrap">
            ...
            <div class="ssf-home-card-grid">
                <?php echo ssf_site_home_value_card('Fartyg', 'Traditionsfartyg som brukas, vårdas och utvecklas.'); ?>
                <?php echo ssf_site_home_value_card('Gemenskap', 'Ett nätverk för fartygsombud, stödmedlemmar och engagerade.'); ?>
                <?php echo ssf_site_home_value_card('Kunskap', 'Erfarenhet, stadgar och samverkan för maritimt kulturarv.'); ?>
            </div>
        </div>
    </section>
...
function ssf_site_home_action_card(string $title, string $text, string $url): string
{
    list($title, $text, $url) = ssf_site_application_destination($title, $text, $url);
    return sprintf('<a class="ssf-home-card ssf-home-card--action" href="%s"><h3>%s</h3><p>%s</p><span>Läs mer</span></a>', esc_url($url), esc_html($title), esc_html($text));
}

function ssf_site_home_value_card(string $title, string $text): string
{
    return sprintf('<div class="ssf-home-card ssf-home-card--value"><h3>%s</h3><p>%s</p></div>', esc_html($title), esc_html($text));
}
...
function ssf_site_application_form_shortcode(): string
{
    ...
    <form class="ssf-form ssf-application-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        ssf_site_form_step('Uppgifter om fartyget', array('<label>Fartygets namn<input name="fartygsnamn" required></label>', '<label>Fartygskategori<select name="fartygstyp" required><option value="">Välj fartygskategori</option><option value="fritidsfartyg">Fritidsfartyg</option><option value="handelsfartyg">Handelsfartyg</option></select><small>Valet avgör årsavgiften.</small></label>', '<label>Är fartyget under restaurering?<select name="restaurering" required><option value="">Välj</option><option value="ja">Ja</option><option value="nej">Nej</option></select></label>', '<label>Kort beskrivning av fartyget<textarea name="fartyg_beskrivning" rows="5" required></textarea></label>', '<label>Länk till webbplats eller mer information<input type="url" name="fartyg_lank"></label>'));
...
function ssf_site_contact_form_markup(string $heading = ''): string
{
    ...
    <form class="ssf-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <?php if (class_exists('SSF_Antispam')) { SSF_Antispam::render('contact'); } ?>
