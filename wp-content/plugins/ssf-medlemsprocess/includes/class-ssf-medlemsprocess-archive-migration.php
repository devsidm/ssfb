...
class SSF_Medlemsprocess_Archive_Migration
{
    ...
    private function render_generic_wizard(): void
    {
        ...
        <div class="wrap ssf-archive-migration">
            ...
            <section id="archive-plan" class="ssf-archive-step"><div class="ssf-archive-step__heading"><span>4</span><div><h2>Migreringsplan och torrkörning</h2><p>Beräknar mål, åtkomst, konflikter och blockerare. Torrkörning gör noll SharePoint-skrivningar; saknade stödda källkolumner planeras för skapande i steg 5.</p></div></div>
                ...
                <?php if ($dry_run) : ?>
                    ...
                    <?php if ($create_columns) : ?><details class="ssf-archive-result" open><summary>Skapar <?php echo esc_html((string) count($create_columns)); ?> saknade kolumner i steg 5</summary><p>Endast stödda kolumner med data i källan skapas i målbiblioteket när du väljer Förbered mål. De läses tillbaka och verifieras innan någon målmapp skapas.</p><ul><?php foreach ($create_columns as $column) : $choice_config = (array) ($column['payload']['choice'] ?? array()); $choice_values = (array) ($choice_config['choices'] ?? array()); ?><li><code><?php echo esc_html((string) ($column['name'] ?? '')); ?></code> — <?php echo esc_html((string) ($column['display_name'] ?? $column['name'] ?? '')); ?> (<?php echo esc_html((string) ($column['type'] ?? 'okänd')); ?>)<?php if ($choice_values) : ?><br><span class="description">Val: <?php echo esc_html(implode(' | ', $choice_values)); ?></span><?php endif; ?></li><?php endforeach; ?></ul></details><?php endif; ?>
            ...
            <section id="archive-prepare" class="ssf-archive-step"><div class="ssf-archive-step__heading"><span>5</span><div><h2>Förbered mål</h2><p>Skapar och verifierar vid behov saknade, stödda källkolumner. Därefter förbereds målroten; för ett direkt biblioteksmål skapas ingen extra mapp.</p></div></div>
                <?php $this->generic_button('ssf_folder_migration_prepare', 'Förbered mål', 'archive-prepare', empty($dry_run['ok'])); ?>
                <?php if (empty($dry_run['ok'])) : ?><p class="description">Knappen aktiveras först när steg 4 visar att torrkörningen är godkänd utan blockerare. Inga mappar skapas innan schemat är skapat och verifierat.</p><?php endif; ?>
                <?php if ($prepared) : ?><p><strong>Slutligt mål:</strong> <?php echo esc_html((string) ($dry_run['destination_path'] ?? '')); ?>. Schemat och målroten är verifierade. Skapade kolumner: <?php echo esc_html((string) count((array) ($prepared['created_columns'] ?? array()))); ?>. Skapade migreringsundermapppar: 0.</p><?php endif; ?>
            </section>
    ...
    public function generic_prepare(): void { $this->generic_execute('ssf_folder_migration_prepare', 'archive-prepare', function ($core, &$state) { return $core->prepare((array) $state['target'], (array) $state['dry_run']); }, 'prepared', 'Schemat är skapat/verifierat och målmappen är förberedd.'); }
