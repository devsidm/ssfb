...
class SSF_Medlemsprocess_SharePoint
{
    ...
    public function repair_schema(): array
    {
        ...
        $diagnostics['message'] = 'Automatisk SharePoint-schemaändring är avstängd. Åtgärda saknade kolumner och Choice-värden manuellt i SharePoint.';
        $diagnostics['manual_action_required'] = true;
        $diagnostics['steps'] = array('repair_schema' => array(
            'label' => 'Uppdatera schema',
            'ok' => false,
            'message' => 'BLOCKED - MANUAL ACTION REQUIRED: WordPress får inte skapa kolumner eller patcha Choice-värden.',
        )) + (array) ($diagnostics['steps'] ?? array());
        return $this->save_schema_diagnostics($diagnostics);
    }
    ...
    {
        return new WP_Error(
            'application_sharepoint_schema_write_disabled',
            'Automatisk skapning eller uppdatering av SharePoint-kolumner är avstängd. Uppdatera schemat manuellt i SharePoint.',
            array('manual_action_required' => true)
        );
    }
    private function schema_repair_error(WP_Error $error, string $stage): WP_Error
    {
        ...
        if (403 === $status) {
            ...
            $data['required_site_role'] = 'manual SharePoint administration only';
            return new WP_Error(
                'application_sharepoint_schema_access_denied',
                'Microsoft Graph nekade åtkomst till SharePoint-schemat. WordPress uppdaterar inte kolumnschema automatiskt; åtgärda schemat manuellt i SharePoint.',
                $data
            );
        }
        return $error;
