...
<section class="ssf-motion-page" aria-labelledby="ssf-motion-heading">
    ...
    <?php else : ?>
        ...
        <form class="ssf-motion-form" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" enctype="multipart/form-data">
            ...
            <p><label for="ssf-motion-content"><?php esc_html_e('Motionstext', 'ssf-member-portal'); ?> <span aria-hidden="true">*</span></label><textarea id="ssf-motion-content" name="content" rows="12" required></textarea></p>
            <p><label for="ssf-motion-files"><?php esc_html_e('Bilagor', 'ssf-member-portal'); ?></label><input id="ssf-motion-files" type="file" name="ssf_motion_files[]" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" multiple><span class="ssf-motion-form__help"><?php echo esc_html(sprintf(__('PDF, DOC eller DOCX. Högst %d MB per fil.', 'ssf-member-portal'), (int) SSF\MemberPortal\Core\Settings::all()['max_upload_mb'])); ?></span></p>
