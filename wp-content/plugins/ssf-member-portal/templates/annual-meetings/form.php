...
<section class="ssf-am-page" aria-labelledby="ssf-am-registration-heading">
    <header class="ssf-am-intro">
        ...
        <h1 id="ssf-am-registration-heading"><?php echo $editing ? esc_html__('Ändra din anmälan', 'ssf-member-portal') : esc_html__('Anmäl dig', 'ssf-member-portal'); ?></h1>
    ...
    <?php if (! $editing && ! $this->meetings->is_registration_open($meeting)) : ?>
        <p class="ssf-am-message"><?php esc_html_e('Anmälan är stängd.', 'ssf-member-portal'); ?></p>
    <?php else : ?>
        <form class="ssf-am-form" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post">
            ...
            <fieldset><legend><?php esc_html_e('1. Dina uppgifter', 'ssf-member-portal'); ?></legend>
                <div class="ssf-am-grid"><p><label for="ssf-am-first-name"><?php esc_html_e('Förnamn', 'ssf-member-portal'); ?> <span aria-hidden="true">*</span></label><input id="ssf-am-first-name" name="first_name" autocomplete="given-name" required value="<?php echo esc_attr($value('first_name')); ?>"></p>
                ...
                <p><label for="ssf-am-phone"><?php esc_html_e('Telefonnummer', 'ssf-member-portal'); ?> <span aria-hidden="true">*</span></label><input id="ssf-am-phone" type="tel" name="phone" autocomplete="tel" required value="<?php echo esc_attr($value('phone')); ?>"></p></div>
            </fieldset>
            ...
            <?php if ($meeting['program']) : ?><fieldset><legend><?php esc_html_e('3. Vilka delar deltar du i?', 'ssf-member-portal'); ?></legend><?php foreach ($meeting['program'] as $item) : if (empty($item['ask'])) { continue; } $selected = ! empty($value('program', array())[$item['key']]); ?><label class="ssf-am-program-option <?php echo ! empty($item['closed']) ? 'is-closed' : ''; ?>"><?php if (! empty($item['closed']) && $selected) : ?><input type="hidden" name="program[<?php echo esc_attr($item['key']); ?>]" value="1"><?php endif; ?><input type="checkbox" name="program[<?php echo esc_attr($item['key']); ?>]" value="1" <?php checked($selected); ?> <?php required(empty($item['optional']) && empty($item['closed'])); ?> <?php disabled(! empty($item['closed'])); ?>><span><strong><?php echo esc_html($item['title']); ?></strong><small><?php echo esc_html(trim($item['date'] . ' ' . $item['start'])); ?><?php if ($item['location']) : ?> · <?php echo esc_html($item['location']); ?><?php endif; ?><?php if ($item['capacity']) : ?> · <?php echo esc_html(sprintf(__('%d platser', 'ssf-member-portal'), $item['capacity'])); ?><?php endif; ?><?php if (! empty($item['closed'])) : ?> · <?php esc_html_e('Valet är stängt', 'ssf-member-portal'); ?><?php endif; ?></small></span></label><?php endforeach; ?></fieldset><?php endif; ?>

            <?php if ($meeting['food_options']) : ?><fieldset><legend><?php esc_html_e('4. Mat och specialkost', 'ssf-member-portal'); ?></legend><p class="ssf-am-help"><?php esc_html_e('Ange endast information som arrangören behöver för att kunna ordna maten.', 'ssf-member-portal'); ?></p><div class="ssf-am-options"><?php foreach ($meeting['food_options'] as $option) : ?><label class="ssf-am-option"><input type="checkbox" name="food[<?php echo esc_attr($option); ?>]" value="1" <?php checked(in_array($option, (array) $value('food', array()), true)); ?>> <?php echo esc_html($option); ?></label><?php endforeach; ?></div><p><label for="ssf-am-food-note"><?php esc_html_e('Annat eller information till köket', 'ssf-member-portal'); ?></label><textarea id="ssf-am-food-note" name="food_note" rows="3"><?php echo esc_textarea($value('food_note')); ?></textarea></p></fieldset><?php endif; ?>
            ...
            <fieldset class="ssf-am-submit"><legend><?php esc_html_e('6. Kontrollera och skicka', 'ssf-member-portal'); ?></legend><p><?php esc_html_e('Uppgifterna används för att administrera SSF:s årsmöte och praktiska arrangemang kring mötet.', 'ssf-member-portal'); ?></p><button class="ssf-am-button" type="submit"><?php echo $editing ? esc_html__('Spara ändringar', 'ssf-member-portal') : esc_html__('Skicka anmälan', 'ssf-member-portal'); ?></button></fieldset>
        </form>
