...
final class Editor
{
    ...
    private function render_program_row(string $index, array $item): void
    {
        ...
        <article class="ssf-am-admin-item" data-ssf-repeater-row>
            ...
            <div class="ssf-am-admin-grid" data-ssf-registration-fields><label>Sista anmälningsdag<input type="datetime-local" name="<?php echo esc_attr($prefix); ?>[deadline]" value="<?php echo esc_attr($this->input_date((int) $item['deadline'])); ?>"></label><label>Max antal<input type="number" min="0" name="<?php echo esc_attr($prefix); ?>[capacity]" value="<?php echo esc_attr((string) $item['capacity']); ?>"><small>0 betyder obegränsat.</small></label></div>
    ...
    {
        return array('key' => '', 'date' => '', 'start' => '', 'end' => '', 'title' => '', 'description' => '', 'location' => '', 'requires_registration' => 0, 'optional' => 1, 'food' => 0, 'closed' => 0, 'manual_open' => 0, 'capacity' => 0, 'deadline' => 0, 'price' => '', 'visible' => 1, 'order' => 0);
    }
