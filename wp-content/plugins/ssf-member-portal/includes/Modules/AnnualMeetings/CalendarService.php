...
final class CalendarService
{
    ...
    public function event_data(array $meeting): array
    {
        ...
        return array(
            ...
            'start' => (new \DateTimeImmutable('@' . $start))->setTimezone(wp_timezone()),
            'end' => (new \DateTimeImmutable('@' . $end))->setTimezone(wp_timezone()),
    ...
    public function urls(array $meeting): array
    {
        ...
        return array(
            ...
            'google' => add_query_arg(array(
                'action' => 'TEMPLATE',
                'text' => $event['title'],
                'dates' => $event['start']->format('Ymd\\THis') . '/' . $event['end']->format('Ymd\\THis'),
            ...
            'outlook' => add_query_arg(array(
                ...
                'rru' => 'addevent',
                'subject' => $event['title'],
                'startdt' => $event['start']->format('Y-m-d\\TH:i:s'),
                'enddt' => $event['end']->format('Y-m-d\\TH:i:s'),
    ...
    public function ics(array $meeting): string
    {
        ...
        $lines = array(
            ...
            'DTSTART;TZID=' . $event['timezone'] . ':' . $event['start']->format('Ymd\\THis'),
            'DTEND;TZID=' . $event['timezone'] . ':' . $event['end']->format('Ymd\\THis'),
